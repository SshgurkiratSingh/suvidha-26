...
const DocumentUpload = ({
  ...
  const startQrSync = () => {
    ...
    setQrValue(
      `http://${window.location.hostname}:3000/sync-upload/${applicationId}`,
    );
  };
  ...
  return (
    <div className="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm relative">
      ...
      <div className="absolute top-6 right-6 flex items-center gap-2">
        <span
          className={`text-[10px] font-bold uppercase ${isConnected ? "text-green-500" : "text-red-500"}`}
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <button
          ...
          className={`p-6 rounded-3xl border-2 transition-all flex flex-col items-center gap-3 ${method === "DIGILOCKER" ? "border-blue-600 bg-blue-50 shadow-md" : "border-gray-100 hover:border-blue-200"}`}
        >
        ...
        <button
          ...
          className={`p-6 rounded-3xl border-2 transition-all flex flex-col items-center gap-3 ${!isConnected ? "opacity-50 cursor-not-allowed" : method === "QR" ? "border-purple-600 bg-purple-50 shadow-md" : "border-gray-100 hover:border-purple-200"}`}
        >
        ...
        <button
          ...
          className={`p-6 rounded-3xl border-2 transition-all flex flex-col items-center gap-3 ${method === "OFFLINE" ? "border-orange-600 bg-orange-50 shadow-md" : "border-gray-100 hover:border-orange-200"}`}
        >
      ...
      <AnimatePresence mode="wait">
        {method === "QR" && (
          <motion.div
            ...
            className="mt-8 flex flex-col md:flex-row items-center gap-8 p-8 bg-purple-50 rounded-3xl border-2 border-dashed border-purple-200"
          >
            ...
            <div className="flex-1">
              <h3 className="text-xl font-bold text-purple-900 mb-2 flex items-center gap-2">
                <Smartphone size={20} /> Mobile Link Ready
              </h3>
              <p className="text-purple-700 text-sm mb-4">
                Scan this code with your phone. Keep this window open while you
                upload files.
              </p>
